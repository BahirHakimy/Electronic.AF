  ...
  return (
    <div className="flex justify-between pt-3 first:pt-1 ">
      <div className="flex grow">
        <img
          src={`${process.env.REACT_APP_imageURL}${info.product?.images[0].thumbnail}`}
          alt="desktop"
          className={` ${style.height} ${style.width} rounded-md  ${style.imageMargin} `}
        />
        <div
          className={`text-sm pt-1 pl-2 ${style.padding} ${style.hover} !grow `}
        >
          <h1 className="font-bold text-sm grow">{info.product.title}</h1>
          <p className="block">{info.product.memory} RAM</p>
          <p>
            {info.product.storage}
            {info.product.storageType}
          </p>
          <p
            hidden={style.priceVisibility}
      ...
      <div hidden={style.deleteVisible} className="pr-5">
        ...
        <select
          name="quantity"
          id="quantity"
          className="rounded mt-7 focus:ring-primary focus:border-primary "
        >
