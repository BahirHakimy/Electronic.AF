...
function Send() {
  ...
  const handleFocus = (e) => {
    ...
    if (e.target.name !== "0" && refInput.current.value === "")
  ...
  const handleSubmit = (e) => {
    ...
    try {
      if (user.email) {
        axios
          .post(`${process.env.REACT_APP_baseURL}auth/checkResetCode/`, {
            email: user.email,
        ...
        setError({
          condition: true,
          message: "Email not available please enter your email again",
        });
      }
    } catch (e) {
      console.log(e);
      ...
      <div className="bg-white md:grid grid-cols-1 items-center h-96 py-24 md:py-0 md:px-24 shadow-md overflow-aut  ">
        ...
        <div className="capitalize mx-3 md:mx-auto max-w-prose">
          ...
          <span className="font-semibold text-gray-400 text-md">
            kindly check your spam folder if the you didn't recive the Security
            code
          </span>
        </div>
        ...
        <div className="mx-4 md:mx-auto my-5">
          <form onSubmit={handleSubmit}>
            <input
              ...
              type="number"
              ref={refInput}
              className="mx-3 md:mx-2 rounded customNumberInput "
              maxLength={1}
              min={1}
              max={9}
              onKeyUp={handleKeyUp}
            ...
            <button
              type="submit"
              className="w-72 md:w-96 block bg-primaryLight rounded-md mb-5 mt-10 ml-10 h-7 font-bold text-white text-xl"
            >
              Submit
            </button>
          </form>
