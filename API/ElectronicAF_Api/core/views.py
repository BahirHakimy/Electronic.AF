    ...
    try:
        ...
        try:
            ...
            if reset_code.is_valid(user):
                ...
                reset_code.save()
                return Response({"detail":"Password reseted successfully"},status=status.HTTP_202_ACCEPTED)
            else:
                return Response({"detail":"Entered resetCode is not valid or expired"},status=status.HTTP_400_BAD_REQUEST)

        except ResetCodes.DoesNotExist:
            return Response({"detail":"Entered resetCode is invalid"},status=status.HTTP_400_BAD_REQUEST)
    except KeyError:
        return Response(
